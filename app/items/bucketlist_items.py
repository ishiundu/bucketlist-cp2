                ...
                if item.bucketlist.creator.user_id is user_id:
                    if item.bucketlist_id is int(bucketlist_id):
                        result = {
                            ...
                            "owner": item.bucketlist.creator.name
                        }
...
class OneBucketlistItem(Resource):
    ...
    def put(self, bucketlist_id, item_id):
        ...
        try:
            ...
            if 'completed' in data.keys():
                single_item.description = data['completed']
            single_item.date_modified = datetime.utcnow()
            db.session.add(single_item)
            db.commit()
            return jsonify({
                'message': "{} item was added successfully".format(
                    single_item.name)})
        except Exception:
            abort(500, message="An error occured, Item was not created")
